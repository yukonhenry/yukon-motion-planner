  ...
  return (
    <section className="panel">
      ...
      <div className="stack">
        {dynamicCount === 0 ? (
          <p className="muted">
            No obstacle is marked <strong>dynamic</strong> — neither stepping nor running would
            change anything. Tick one in the obstacle list.
          </p>
        ) : (
          <p className="muted hint">
            {dynamicCount === 1 ? "One obstacle" : `${dynamicCount} obstacles`} can move. Each
            environment tick jitters one corner of {dynamicCount === 1 ? "it" : "each"}; the planner
            replans on its own clock.
          </p>
        )}
          ...
          <div className="stack">
            ...
            <p className="muted">
              {run.route === null ? (
                ...
                <>
                  Move {run.planTick} · {run.reachable ? `cost ${run.cost}` : "no route"} ·{" "}
            ...
            <p className="muted hint">
              The clock is on the server. Closing this tab leaves the run going — reopening the grid
              rejoins it. Nothing is saved either way.
            </p>
          </div>
        ) : (
          <div className="stack">
            ...
            <div className="row">
              <button type="button" onClick={onStart} disabled={runPending || runBlocked !== null}>
                {runPending ? "Starting…" : "Run simulation"}
              </button>
            </div>
            <p className="muted hint">
              Runs the environment and the replanner as two backend tasks, each on its own frequency
              — the grid's <code>sim_interval</code> for the world, the start request's{" "}
              <code>replan_interval</code> for the planner.
            </p>
          </div>
        )}
