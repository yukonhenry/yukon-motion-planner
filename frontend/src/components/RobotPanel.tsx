  ...
  return (
    <section className="panel">
        ...
        <ul className="plan-list">
          {robots.map((robot) => (
            <li key={robot.id} className={robot.id === editing ? "selected" : ""}>
              <button type="button" className="subtle plan-list__pick" onClick={() => edit(robot)}>
                {robot.name}{" "}
                <span className="muted">
                  {robot.capabilities.max_velocity} cells/tick · {robot.capabilities.task_interval}s
                </span>
              </button>
              <button type="button" className="danger subtle" onClick={() => onDelete(robot.id)}>
                Delete
              </button>
            </li>
          ))}
      ...
      <div className="stack">
        ...
        <p className="muted hint">Cells travelled each time the robot moves.</p>
        ...
        <p className="muted hint">
          Seconds between moves — the robot's own clock, separate from how often the world's
          obstacles move.
        </p>
